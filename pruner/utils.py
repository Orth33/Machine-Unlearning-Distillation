...
def _get_model_num_classes(model):
    ...
    raise AttributeError("Could not infer the classifier output dimension from model")


# Pruning operation
def pruning_model(model, px):
    print("Apply Unstructured L1 Pruning Globally (all conv layers)")
    parameters_to_prune = [(m, "weight") for m in _get_prunable_modules(model)]
...
def pruning_model_structured(model, px):
    print("Apply Unstructured L1 Pruning Globally (all conv layers)")
    # parameters_to_prune =[]
    for m in _get_prunable_modules(model):
        prune.ln_structured(
            m,
            name="weight",
            amount=px,
...
def check_sparsity(model):
    ...
    if sum_list == 0:
        print("no conv weight for calculating sparsity")
        remain_weight_ratie = None
    else:
        remain_weight_ratie = 100 * (1 - zero_sum / sum_list)
        print("* remain weight ratio = ", remain_weight_ratie, "%")
...
def snip_importance_score(
    ...
    x, y = fetch_data(dataloader, _get_model_num_classes(model), samples_per_class)
