...
def main():
    ...
    criterion = build_training_criterion(args, train_loader.dataset, device)
    ...
    if args.prune_type == "lt":
        ...
        if args.rewind_epoch > 0:
            ...
            print(
                "[rewind_lt] --rewind_epoch was not set; falling back to the initial model weights."
            )
            initalization = deepcopy(model.state_dict())
    else:
    ...
    for state in range(start_state, args.pruning_times):
        ...
        if initalization is None:
            raise ValueError(
                "Rewind weights were never captured. Set --rewind_epoch to a positive "
                "epoch within training, or use --prune_type lt/pt."
            )
